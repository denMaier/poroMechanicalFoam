#!/usr/bin/python3
"""Make matched tau--Fo plots for undrained footing sweeps."""
...
from undrained_footing_sweep import field_to_grid, final_pressure, logarithmic_edges
...
def plot_selected_pressures(
    ...
    for directory, data in zip(directories, datasets):
        fields = []
        for key in selected:
            p = field_to_grid(final_pressure(directory / data[key]["case"]))
            fields.append(
                (p - np.mean(p))
