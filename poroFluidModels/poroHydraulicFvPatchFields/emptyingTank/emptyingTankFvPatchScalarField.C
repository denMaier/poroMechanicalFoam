...
void Foam::emptyingTankFvPatchScalarField::updateCoeffs()
{
    ...
    const fvsPatchField<scalar> &flux =
        ...
        const tmp<scalarField> nTmp(
            patch().nf() & vector(pFM.poroHydraulic().gamma().value()).normalise()
        );
        const scalarField& n_ = nTmp();
        const tmp<scalarField> zTmp(
            patch().Cf() & vector(pFM.poroHydraulic().gamma().value()).normalise()
        );
        const scalarField& z_ = zTmp();
