...
defineTypeNameAndDebug(seismicLinGeomSolid, 0);
addToRunTimeSelectionTable(solidModel, seismicLinGeomSolid, dictionary);


// * * * * * * * * * * *  Private Member Functions * * * * * * * * * * * * * //


void seismicLinGeomSolid::predict()
{
...
seismicLinGeomSolid::seismicLinGeomSolid
(
    Time& runTime,
    const word& region
)
...
bool seismicLinGeomSolid::evolve()
{
        ...
        do
        {
            ...
            (
             ...
             == fvm::laplacian(impKf_, DD(), "laplacian(DDD,DD)")
              ...
              + fvOptions_(onebyTime.ref(),rho(), DD()) // used deltaT to get dimensions of stress
            );

            fvOptions_.constrain(DDEqn);
...
tmp<vectorField> seismicLinGeomSolid::tractionBoundarySnGrad
(
