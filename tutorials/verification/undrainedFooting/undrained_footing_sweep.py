#!/usr/bin/python3
"""Sweep the fully undrained first increment of a strip-footing problem."""
...
def field_to_grid(field: np.ndarray) -> np.ndarray:
    """Reconstruct the physical grid from the two block-major cell ranges."""
    split = NX_FOOTING * NZ
    footing = field[:split].reshape(NZ, NX_FOOTING)
    free = field[split:].reshape(NZ, NX_FREE)
    return np.concatenate((footing, free), axis=1)


def grid_to_field(grid: np.ndarray) -> np.ndarray:
    """Convert a physical grid to the cell ordering written by blockMesh."""
    return np.concatenate(
        (grid[:, :NX_FOOTING].reshape(-1), grid[:, NX_FOOTING:].reshape(-1))
    )


def mode_values(kind: str) -> np.ndarray:
    x = np.tile((-1.0) ** np.arange(NX), (NZ, 1))
    z = np.tile(((-1.0) ** np.arange(NZ))[:, None], (1, NX))
    grid = {"checkerboardX": x, "checkerboardZ": z, "checkerboardXZ": x * z}[kind]
...
def write_mode(path: pathlib.Path, name: str, data: np.ndarray) -> None:
    ...
    path.write_text(
        ...
        "boundaryField\n"
        "{\n"
        "    \"left|right|bottom|topLoaded|topFree\" { type zeroGradient; }\n"
...
def prepare_case(
    ...
    set_entry(
        d_path,
        "boundaryField/topLoaded/traction",
        f"uniform (0 0 {footing_traction:.16g})",
    )
    return target
...
def analyse(
    ...
    row: dict[str, object] = {
        ...
        "Ss": point.storage,
        "delta_t": point.dt,
        "footing_traction": footing_traction,
...
def plot_pressures(rows: list[dict[str, object]], runs: pathlib.Path, path: pathlib.Path) -> None:
    ...
    for row in rows:
        try:
            p = field_to_grid(final_pressure(runs / str(row["case"])))
        except (OSError, RuntimeError, ValueError):
            continue
...
def main() -> int:
    ...
    parser.add_argument(
        "--footing-traction",
        type=float,
        default=DEFAULT_FOOTING_TRACTION,
        help="total vertical traction on topLoaded (negative is compressive)",
    )
    ...
    if args.footing_traction >= 0.0:
        parser.error("--footing-traction must be negative for compression")
